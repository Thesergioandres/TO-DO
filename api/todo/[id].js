...
async function getTodoById(req, res) {
  try {
    ...
    if (todo.user_id !== userId && todo.userId !== userId) {
      return sendResponse(res, createErrorResponse('Access denied to this todo', 403));
    }
    ...
    return sendResponse(res, createSuccessResponse(formattedTodo, 'Todo retrieved successfully'));
...
async function updateTodo(req, res) {
  try {
    ...
    const { userId } = req.user;
    const { id } = req.query;
    const { title, description, priority, category, tags, due_date, completed } = req.body;
    
    console.log(`[TODO UPDATE] Updating todo ${id} for user: ${userId}`);
    console.log(`[TODO UPDATE] Data:`, { title, description, priority, category, tags, due_date, completed });
    ...
    return sendResponse(res, createSuccessResponse(formattedTodo, 'Todo updated successfully'));
...
async function deleteTodo(req, res) {
  try {
    ...
    if (!deleted) {
      return sendResponse(res, createErrorResponse('Failed to delete todo', 500));
    }
    
    console.log(`[TODO DELETE] Deleted todo ${id} for user ${userId}`);
    return sendResponse(res, createSuccessResponse({ id }, 'Todo deleted successfully'));
...
module.exports = async (req, res) => {
  console.log(`[TODO ID API] ${req.method} request received for todo: ${req.query.id}`);
  ...
  try {
    switch (req.method) {
      ...
      default:
        ...
        return sendResponse(res, createErrorResponse(`Method ${req.method} Not Allowed`, 405));
    }
  } catch (error) {
    return handleApiError(error, req, res);
  }
