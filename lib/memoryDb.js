...
function createTodo(todoData) {
  const todo = {
    ...
    tags: typeof todoData.tags === 'string' ? todoData.tags : JSON.stringify(todoData.tags || []),
...
function getTodosByUserId(userId, filters = {}) {
  let todos = database.todos.filter(t => t.user_id === userId && !t.deleted_at);
...
function updateTodo(todoId, updateData) {
  const todoIndex = database.todos.findIndex(t => t.id === todoId && !t.deleted_at);
  if (todoIndex === -1) {
...
function hardDeleteTodo(todoId) {
  ...
  if (todoIndex === -1) {
    logOperation('HARD_DELETE_TODO_FAILED', { todoId, reason: 'Todo not found' });
    return false;
  }
...
function getDatabaseStatistics() {
  ...
  const completedTodos = database.todos.filter(t => t.completed && !t.deleted_at).length;
...
function importDatabase(importData) {
  if (!importData || !importData.users || !importData.todos) {
    logOperation('DATABASE_IMPORT_FAILED', { reason: 'Invalid import data structure' });
